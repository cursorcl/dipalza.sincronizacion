...
public class HojaRutaExcel extends EventEmisor {
	...
	private class OTRegistro {
		...
		public String toString() {
			return "[ruta=" + ruta + ", articulo=" + articulo + ", cantidad=" + cantidad + ", totalLinea=" + totalLinea
					+ "]";
		}
	}
	...
	/**
	 * Inicializa el procesamiento estableciendo la lista de rutas y articulos
	...
	/**
	 * Obtiene las ventas realizadas en una fecha determinada, solo las de es
	 * día.
	 * 
	 * @param fecha
	 *            Fecha en la que se quiere obtener las ventas.
	 * @return Listado de las ventas realizadas en ese día.
	 * @throws SQLException
	 *             Excepción si hay una falla en la búsqueda de información.
	 */
	...
	private void crearDocExcel(final Workbook wbWork, Date fecha) {
		...
		String path = Utils.getDefaultDirectory().toString() + File.separator + "HojaRuta-" + System.currentTimeMillis()
				+ ".xls";
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "No se pudo grabar archivo " + path, "Problemas al grabar",
					JOptionPane.ERROR_MESSAGE);
		} finally {
			try {
				fileOut.close();
			} catch (final IOException e) {
				JOptionPane.showMessageDialog(null, "No se pudo cerrar archivo " + path, "Problemas al cerrar archivo",
						JOptionPane.ERROR_MESSAGE);
			}
		}
