...
public class DataSQL extends EventEmisor implements IProcessor {
    ...
    public VectorClientes getClientes(String gVend, String gRuta,
	    String gRutaAdicional) {
	...
	try {
	    ...
	    strSelect = "select a.articulo,  a.descripcion, b.stock, c.ventaNeto, a.unidad, a.costo, a.porcIla from articulo a , artxlocal b , precios c  where c.articulo = a.articulo  and c.codigolista = '000'  and a.articulo = b.articulo and b.local ='000' and len(a.articulo) = 3 order by left(a.articulo, 2), a.articulo";
		...
		Producto p = new Producto(articulo, idProducto, descripcion,
			stock, ventaNeta, unidad, "", costo, ila);
		vProductos.add(p);
	...
	try {
	    ...
	    ResultSet res = pstmt.executeQuery();
	    while (res.next()) {
		v.addElement(new ListaCorrelativos(res.getString("articulo"),
			res.getInt("numero"), res.getFloat("peso")));
	    }
	    res.close();
	    pstmt.close();
	} catch (SQLException e) {
	...
	try {
	    ...
	    pstmt.setString(6,
		    FormatoNumeros.putStrAtBegin(cl.getRut(), '0', 10));
	    if (estado == Constantes.CLIENTE_NUEVO) {
    ...
    public String setVentas(Vector<?> encabezado, Vector<?> itemes,
		...
		for (int n = 0; n < encabezado.size(); ++n) {
			...
			while (nItemes < filasV.size()) {
				...
				if (strCondicion.contains("CREDITO")) {
				    ...
				    registro.setVenta(v.getNeto()
					    * (1 + v.getPorcentajeIva()));
				    ListadoCreditos.getInstance().add(registro);
				}
			    }
			    for (j = 1; (j <= 25) && (nItemes < filasV.size()); j++) {
				ItemVenta iv = (ItemVenta) filasV
					.elementAt(nItemes);
				ResultDetalle r = factura.insertDetalleVenta(
					iv, id, j, true);
				if ((r.getResult() == 1)
					|| (r.getResult() == 2)) {
				    allItems = false;
				    Producto pr = factura.getDatosProducto(iv
					    .getArticulo());
				    mmiFaturas.addItemFallido(nroFactura
					    + " : " + pr.getArticulo() + " "
					    + pr.getNombre() + " "
					    + r.getFaltante());
				}
			    ...
			    factura.insertTotalDocumento(id, neto,
				    v.getPorcentajeIva(), ila);
			    ...
			    FacturaItemModel fModel = getFacturaModel(
				    nroFactura, allItems, v, neto);
	...
	mmiFaturas
		.message("Se ha concluido el ingreso de facturas", "FACTURAS");
	return nroFactura;
    }

    private String setVenta(EncabezadoVenta encabezado, ItemesVenta itemes,
	    FechaFormateada fecha) {
	mmiFaturas.setVisible(true);
	int nItemes = 0;
	int j = 0;
	...
	if ((itemes != null) && (itemes.size() > 0)) {
		...
		while (nItemes < itemes.size()) {
		    ...
		    ila = factura.insertIla(nroFactura, id);
		    factura.insertTotalDocumento(id, neto,
			    encabezado.getPorcentajeIva(), ila);

		    factura.insertCtaDcto(nroFactura, encabezado, neto, ila);
		    factura.insertDatosCliente(rut, id, condicion, dias);
		    factura.insertFolios(nroFactura);
		    FacturaItemModel fModel = getFacturaModel(nroFactura,
			    allItems, encabezado, neto);
		    mmiFaturas.addItemVenta(fModel);
    ...
    private FacturaItemModel getFacturaModel(String nroFactura,
	    boolean allItems, EncabezadoVenta e, float neto) {
	...
	return new ItemVenta((short) 0, pr.getArticulo(), 1.0F, pr.getPrecio(),
		0.0F, 0.0F);
    }
	...
	String nFactura = "";
	FechaFormateada fecha = new FechaFormateada(
	...
	try {
	    ...
	    if (oId != null) {
		pstmt = con
			.prepareStatement("select * from EncabezaDocumento E, TotalDocumento T where E.ID = ? and E.id = T.id");
		...
		notify("Generado Encabezado Documento con factura = "
			+ oFactura);

		pstmt = con
			.prepareStatement("select * from DETALLEDOCUMENTO WHERE ID = ?");
		pstmt.setString(1, oId);
		r = pstmt.executeQuery();
		...
		notify("Proceso de generaciÃ³n de nueva Factura = " + nFactura
			+ " finalizado");
	    }
	} catch (SQLException e) {
	    notify("Ha Fallado la generaciónn de la factura = " + nFactura);
	    e.printStackTrace();
	    nFactura = "";
	}
	return nFactura;
    ...
    public void addVenta(EncabezadoVenta encabezado, ItemesVenta itemes,
	    FechaFormateada fecha) {
