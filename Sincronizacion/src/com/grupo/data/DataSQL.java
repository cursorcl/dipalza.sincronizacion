...
public class DataSQL extends EventEmisor implements IProcessor {
  ...
  private static final String SQL_INS_MSOCLIENTES =
      "insert into msoclientes(razon, direccion, ciudad, comuna, telefono, rut, vendedor, cobrador, ruta, frecuencia, ingreso) values (?, ?, ?, ?, ?, ?, ?, ?, ?, '002', ?)";
  private static final String SQL_UPD_MSOCLIENTES =
      "update msoclientes set razon = ?, direccion= ?, ciudad = ?, comuna = ?, telefono = ? where rut = ?";
  ...
  public VectorClientes getClientes(String gVend, String gRuta, String gRutaAdicional) {
    ...
    String ruta = null;
    strSelect =
        "select * from msoclientes where  vendedor= ? and (ruta = ? or ruta =  ?) order by razon";
    try {
  ...
  public List<ProductosNumerados> getArticulosNumerados() {
    ...
    strSelect =
        "select n.articulo, n.numero, a.descripcion, n.peso  from Numerados n, Articulo a where n.articulo = a.articulo order by n.articulo, n.numero";
    try {
      PreparedStatement pstmt = con.prepareStatement(strSelect);
      ResultSet res = pstmt.executeQuery();
      while (res.next()) {
  ...
  public Vector<ListaCorrelativos> getCorrelaModificar() {
    ...
    try {
      ...
      ResultSet res = pstmt.executeQuery();
      while (res.next()) {
        v.addElement(new ListaCorrelativos(res.getString("articulo"), res.getInt("numero"), res
            .getFloat("peso")));
      }
      res.close();
      pstmt.close();
  ...
  public String clonarFactura(String oFactura) {
    ...
    try {
      ...
      PreparedStatement pstmt =
          con.prepareStatement("SELECT E.id FROM ENCABEZADOCUMENTO AS E where E.numero = ?");
      ...
      if (oId != null) {
        pstmt =
            con.prepareStatement("select * from EncabezaDocumento E, TotalDocumento T where E.ID = ? and E.id = T.id");
