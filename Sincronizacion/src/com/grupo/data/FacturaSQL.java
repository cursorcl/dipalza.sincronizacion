...
public class FacturaSQL {
	...
	String strSelect = "Select a.descripcion, p.ventaneto, x.stock, a.costo, a.porcIla from articulo as a, artxlocal as x, precios as p where a.articulo = ? and x.articulo = a.articulo and x.local = '000' and p.articulo = a.articulo and p.codigolista = '000'";
	try {
	    PreparedStatement pstmt = this.con.prepareStatement(strSelect);
	    pstmt.setString(1, articulo);
	    ResultSet res = pstmt.executeQuery();
	    if (res.next())
		p = new Producto(articulo, s, res.getString("descripcion"),
			res.getFloat("stock"), res.getFloat("ventaneto"), "",
			"", res.getFloat("costo"), res.getFloat("porcIla"));
	...
	try {
	    ...
	    ResultSet res = pstmt.executeQuery();
	    while (res.next()) {
		Integer numero = Integer.valueOf(Integer.valueOf(
			res.getString(1)).intValue());
		result = FormatoNumeros.putZeroesAtBegin(
	...
	try {
	    ...
	    pstmt.setString(2,
		    FormatoNumeros.putZeroesAtBegin(condicionVenta, 3));
    ...
    public ResultDetalle insertDetalleVenta(ItemVenta pVenta, String id,
	    int fila, boolean validaStock) {
	...
	try {
	    ...
	    if (pr == null) {
		log.error(String.format("El prodcuto %s no existe en la BD",
			new Object[] { articulo }));
	    } else {
		...
		if (validaStock) {
		    PreparedStatement stmNumerado = this.con
			    .prepareStatement("select * from Numerados where articulo = ?");
			...
			if (pr.getStock() <= 0.0F) {
				...
				ventNeto = venta
					* pr.getPrecio()
					* (1.0F - pVenta.getDescuento() / 100.0F);
				result.setResult(1);
			    }
			    procesa = true;
			}
			...
			procesa = nVenta != nV.length;
			if ((nVenta > 0) && (nVenta < nV.length)) {
			    result.setResult(1);
			} else if (!procesa) {
			    result.setResult(2);
			    procesa = false;
    ...
    public boolean insertTotalDocumento(String id, float total, float porIva,
	    float ila) {
	boolean result = false;
    ...
    public boolean insertCtaDcto(String factura, EncabezadoVenta e, float neto,
	    float ila) {
	boolean result = false;
	float comision = 0.0F;
	...
	try {
	    PreparedStatement pstmtSel = this.con
		    .prepareStatement("select comision from msovendedor where codigo = ?");
	    ...
	    PreparedStatement pstmtIns = this.con
    ...
    public boolean insertDatosCliente(String rut, String id, String condicion,
	    int dias) {
	boolean result = false;
	try {
	    PreparedStatement pstmt = this.con
		    .prepareStatement("SELECT m.telefono, m.ruta, m.ciudad, m.comuna, m.razon, m.vendedor, v.comision, m.giro, m.vendedor as cobrador, m.direccion FROM msovendedor v, msoclientes m where v.codigo = m.Vendedor and m.rut = ?");
	    pstmt.setString(1, rut);
	    ResultSet r = pstmt.executeQuery();
	    if (r.next()) {
		PreparedStatement pstmtIns = this.con
			.prepareStatement("insert into datoscliente values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	try {
	    PreparedStatement pstmt = this.con
		    .prepareStatement("INSERT INTO folios (NUMERO, TIPO) VALUES (? , ?)");
	...
	try {
	    PreparedStatement pstmt = this.con
		    .prepareStatement("select numero from encabezadocumento where id = ?");
    ...
    private void rebajaVenta(String articulo, float rebajaReal)
	    throws SQLException {
	PreparedStatement pstmt = this.con
	...
	int numDiasCondicionVenta = getNumDiasCondicionVenta(pEncabezado
		.getCondicionVenta());
	...
	try {
	    PreparedStatement pstmt = this.con
		    .prepareStatement(SQL_SEL_DATOS_CLIENTE);
	    pstmt.setString(1, rut);
	    ResultSet r = pstmt.executeQuery();
	    if (r.next()) {
		ruta = r.getString(1);
	...
	try {
		...
		p = new Producto(codigo, s, descripcion, 0.0F, valor, "", "",
			valor, 0.0F);
	    }
	} catch (SQLException e) {
	    e.printStackTrace();
	...
	try {
	    ...
	    ResultSet r = pstmt.executeQuery();
	    while (r.next()) {
		PreparedStatement pstmtInsert = this.con
			.prepareStatement("insert into MSOSTVENTASILA values (?, ?, ?, ?, ?, '', 0)");
