...
public class FacturaSQL {
	...
	public Producto getDatosProducto(String articulo) {
		...
		String strSelectWhithILA = "Select a.descripcion, p.ventaneto, a.costo, b.valor as porcIla from articulo as a, precios as p, MSOSTTABLAS as b where a.articulo = ? and p.articulo = a.articulo  and a.codigoila = b.codigo and b.descripcion like '%ILA%' and p.codigolista = '"
				+ lista + "'";

		String strSelectWhithoutILA = "Select a.descripcion, p.ventaneto, a.costo, a.porcIla from articulo as a, precios as p where a.articulo = ? and p.articulo = a.articulo and p.codigolista ='"
				+ lista + "'";
		Producto p = null;
		try {
			...
			PreparedStatement pstmt4 = this.con.prepareStatement(strSelectWhithILA);
			pstmt4.setString(1, articulo);
			ResultSet res4 = pstmt4.executeQuery();
			if (res4.next()) {
				p = new Producto(articulo, s, res4.getString("descripcion"), stock, res4.getFloat("ventaneto"), "", "",
						res4.getFloat("costo"), res4.getFloat("porcIla"));
			}
			res4.close();
			pstmt4.close();
			if (p == null) {
				PreparedStatement pstmt5 = this.con.prepareStatement(strSelectWhithoutILA);
				pstmt5.setString(1, articulo);
				ResultSet res5 = pstmt5.executeQuery();
				if (res5.next()) {
					p = new Producto(articulo, s, res5.getString("descripcion"), stock, res5.getFloat("ventaneto"), "",
							"", res5.getFloat("costo"), res5.getFloat("porcIla"));
				}
				res5.close();
				pstmt5.close();
	...
	public String getId() {
		...
		try {
			...
			while (res.next()) {
				...
				result = FormatoNumeros.putZeroesAtBegin((numero = Integer.valueOf(numero.intValue() + 1)).intValue(),
						10);
			}
	...
	private int getNumDiasCondicionVenta(byte condicionVenta) {
		...
		try {
			PreparedStatement pstmt = this.con
					.prepareStatement("select valor from msosttablas where tabla = ? and codigo = ?");
	...
	public ResultDetalle insertDetalleVenta(ItemVenta pVenta, String id, int fila) {
		...
		float ventaNetoLinea = pVenta.getNeto();
		try {
			...
			if (pr != null) {
				...
				PreparedStatement stmNumerado = this.con
						.prepareStatement("select * from articulosnumerados where  articulo = ?");
				...
				if (!isNumerated) {
					...
					if (pr.getStock() <= 0.0F) {
						...
						if (difStock < 0.0F) {
							...
							venta = pr.getStock();
							ventaNetoLinea = venta * pVenta.getPrecio() * (1.0F - pVenta.getDescuento() / 100.0F);
							result.setResult(1);
						}
						procesa = true;
					}
				...
				if (procesa) {
					...
					pstmt.setFloat(2, ventaNetoLinea);
					...
					pstmt.executeUpdate();
					result.setTotal(ventaNetoLinea);
				}
			} else {
...
//		float iva = e.getPorcentajeIva() / 100.0F * neto;
		...
		try {
			PreparedStatement pstmtSel = this.con.prepareStatement("select comision from msovendedor where codigo = ?");
			...
			PreparedStatement pstmtIns = this.con.prepareStatement(
	...
	public boolean insertDatosCliente(String rut, String id, String condicion, int dias, String codigo) {
		...
		try {
			...
			PreparedStatement pstmt = this.con.prepareStatement(
					"SELECT m.telefono, m.ruta, m.ciudad, m.comuna, m.razon, m.vendedor, v.comision, m.giro, m.vendedor as cobrador, m.direccion FROM msovendedor v, msoclientes m where v.codigo = m.Vendedor and m.rut = ? and m.codigo=?");
			...
			if (r.next()) {
				PreparedStatement pstmtIns = this.con.prepareStatement(
						"insert into datoscliente (Telefono, Despacho, ComisionCobrador, Ruta, TipoVenta, Transporte, Ciudad, Comuna, Razon, Lista, Dias, CondicionVenta, tipoid, Vendedor, ComisionVendedor, Giro, Cobrador, Direccion, Id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public boolean insertFolios(String nroFactura) {
		...
		try {
			PreparedStatement pstmt = this.con
					.prepareStatement("INSERT INTO folios (NUMERO, TIPO, TIPO1) VALUES (? , ?, ?)");
	...
	public String getNumeroFactura(String id) {
		String nro = null;
		try {
			PreparedStatement pstmt = this.con.prepareStatement("select numero from encabezadocumento where id = ?");
	...
	private void rebajaVenta(String articulo, float rebajaReal) throws SQLException {
		...
		PreparedStatement pstmt = this.con
				.prepareStatement("update artxlocal set stock = stock - ? where articulo = ?");
	...
	public float insertIla(String nroFactura, String id) {
		...
		try {
			...
			while (r.next()) {
				...
				PreparedStatement pstmtInsert = this.con.prepareStatement(
						"insert into MSOSTVENTASILA (tipo, TIPO1, codigo, valor, numero, ila) values (?, ?, ?, ?, ?, ?)");
				pstmtInsert.clearParameters();
				// Se agrega redondeo
				float valor = Math.round(r.getFloat("Valor"));
				ila += valor;
				pstmtInsert.setString(1, r.getString("TipoId"));
				pstmtInsert.setString(2, tipo1);
				pstmtInsert.setString(3, r.getString("CodigoIla"));
				pstmtInsert.setFloat(4, valor);
				pstmtInsert.setString(5, nroFactura);
				pstmtInsert.setInt(6, r.getInt("PorcIla"));
	...
	public String getCondicionVenta(int nDias) {
		...
		try {
			PreparedStatement pstmt = this.con
					.prepareStatement("Select codigo from msosttablas where tabla = ? and valor = ?");
			pstmt.setString(1, "009");
	...
	public static void main(String[] args) {
		...
		String sqlINVDETALLEPARTES_MINUS = "select sum(cantidad) as stock from invdetallepartes  i, INVENCABEZAPARTES e where articulo = ?   and i.Id =  e.Id  and i.Tipoid = 18 and i.Local ='000'";
		String sqlDETALLEDOCUMENTOCredito = "select sum(cantidad) as stock from detalledocumento  d, encabezadocumento e where d.articulo = ? and d.local = '000'  and d.tipoid = '09' and d.id = e.id  and e.vigente = 1";
		String sqlDETALLEDOCUMENTODebito = "select sum(cantidad) as stock from detalledocumento  d, encabezadocumento e where d.articulo = ? and d.local = '000'  and d.tipoid = '06' and d.id = e.id  and e.vigente = 1 ";

		String lista = "001";
		String strSelect = "Select a.descripcion, p.ventaneto, a.costo, a.porcIla from articulo as a, precios as p where a.articulo = ? and p.articulo = a.articulo and p.codigolista ='"
				+ lista + "'";
