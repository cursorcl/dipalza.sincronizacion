	...
	public void handle(EventObject event) {
		if (event instanceof ConnectionClientEvent) {
			...
			case MSG_ENCABEZADO:
				if (this.esperandoMensaje.equals(EMessagesTypes.MSG_ENCABEZADO)) {
					try {
						...
						log.debug("Encabezado Factura: "
								+ this.encabezado.getNombreCliente());
						...
						log.error("Encabezado Factura con error: "
								+ encabezado.getNombreCliente());
					}
					return;
				} else {
					log
							.error("Ha llegado un encabezado de factura desincronizado.");
			...
			case MSG_NROITEMESVENTA:
				if (esperandoMensaje.equals(EMessagesTypes.MSG_NROITEMESVENTA)) {
					...
					log.debug(String.format(
							"A la espera de %d registros de venta",
							countItemsVenta));
					itemsVentaLlegados = 0;
					esperandoMensaje = EMessagesTypes.MSG_ITEMVENTA;
				} else {
					log
							.error("Ha llegado cantidad de itemes de venta desincronizado.");
			...
			case MSG_ITEMVENTA:
				if (esperandoMensaje.equals(EMessagesTypes.MSG_ITEMVENTA)) {
					...
					if (itemsVentaLlegados < countItemsVenta) {
						log.debug(String.format("Item de Venta:%s %s %f  $%f",
								item.getCodigoProducto(), item.getArticulo(), item.getCantidad(),
								item.getNeto()));
						itemsVentaLlegados++;
						itemes.add(item);
					}
					...
					log
							.error("Ha llegado un registro de venta desincronizado.");
			...
			case MSG_VECTORVENTAS:
				if (esperandoMensaje.equals(EMessagesTypes.MSG_ENCABEZADO) || esperandoMensaje.equals(EMessagesTypes.MSG_VECTORVENTAS)) {
